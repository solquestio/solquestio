...
exports.QUEST_DEFINITIONS = void 0;
const express_1 = __importDefault(require("express"));
const authMiddleware_1 = require("../middleware/authMiddleware"); // Import protect middleware
...
exports.QUEST_DEFINITIONS = QUEST_DEFINITIONS;
...
router.get('/paths', (req, res) => {
    try {
        ...
        const getPathDetails = (pathIdToProcess) => {
            let count = 0;
            let totalXp = 0;
...
router.post('/check-balance', authMiddleware_1.protect, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const publicKey = new web3_js_1.PublicKey(user.walletAddress);
        ...
        if (balanceSOL >= balanceThresholdSOL) {
            ...
            const updatedUserDoc = yield User_1.default.findByIdAndUpdate(userId, {
                $push: { completedQuestIds: FUND_WALLET_QUEST_ID, xpEvents: xpEvent },
                $inc: { xp: finalReward }
            }, { new: true });
            if (updatedUserDoc) {
                const updatedUser = updatedUserDoc; // Cast for type safety
...
router.get('/', authMiddleware_1.protect, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const user = yield User_1.default.findById(userId).select('completedQuestIds');
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
        ...
        const questsWithStatus = quests.map(quest => (Object.assign(Object.assign({}, quest), { isCompleted: user.completedQuestIds.includes(quest.id) })));
...
router.get('/path/:pathId', authMiddleware_1.protect, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (!req.user) {
        return res.status(401).json({ message: 'Not authorized' });
    }
    const { userId } = req.user;
    ...
    try {
        ...
        // Map quests and add completion status
        const questsWithStatus = pathQuests.map(quest => (Object.assign(Object.assign({}, quest), { isCompleted: user.completedQuestIds.includes(quest.id) })));
        res.status(200).json(questsWithStatus);
    }
    catch (error) {
...
router.post('/verify-transaction', authMiddleware_1.protect, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (!req.user || !req.user.userId)
        return res.status(401).json({ message: 'Not authorized' });
    const { userId } = req.user;
    ...
    const verifiedQuestId = questId; // Change this to string type
    ...
    try {
        ...
        const correctAnswer = questDefinition.correctAnswer;
...
router.post('/verify-answer', authMiddleware_1.protect, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (!req.user || !req.user.userId) {
        return res.status(401).json({ message: 'Not authorized' });
    }
    const userId = req.user.userId;
    ...
    const questDefinition = QUEST_DEFINITIONS[questId];
    if (!questDefinition)
    ...
    try {
        ...
        const updatedUserDoc = yield User_1.default.findByIdAndUpdate(userId, { $push: { completedQuestIds: questId, xpEvents: xpEvent }, $inc: { xp: finalReward } }, { new: true }).select('-__v');
        if (!updatedUserDoc)
...
router.post('/wormhole/verify-tx', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
